...
def find_all_contacts(complex, pro_interface_dict, pep_interface_dict, composition_counter_pro, composition_counter_pep, check, one_model, within_model, between_models, cut_off, no_interchain_contacts_file, which_is_protein_dict, pairings_file):
    translation_dict = {}
            ...
            for distance in list_of_distances:
                ...
                if distance < cut_off:
                    ...
                    if position_chain_id[-1] != chain: #only intrested in interactions between two different chains
                        ...
                        if res == "W" and contact_res == "P" :
                            info = str(res) + " "+  str(pos)+" " +str(chain)+ " is in contact with "+ str(contact_res)+ " at "+ str(position_chain_id)+ " Distance: "+ str(distance)+ " protein chain: "+ str(protein_chain)
                            row = complex + "  "+ info + '\n'
                            pairings_file.write(row)



                        if chain==protein_chain: #if contact is from protein side save in protein dict else save in peptide dict
...
def calc_composition(composition_counter_pro, composition_counter_pep, check):
    ...
    tot_number_involved_residues_pep = len(number_of_residues_list_pep)

    print("Number of residues on peptide side of interface:", tot_number_involved_residues_pep)
    tot_number_involved_residues = tot_number_involved_residues_pep + tot_number_involved_residues_pro
...
####################################################################################################################### protein side
    ...
    csv_dict = {}
    for entry in zip(x_list_pep, y_list_pro, y_list_pep):
        csv_dict[entry[0]] = (entry[1], entry[2])

    df_csv = pd.DataFrame.from_dict(csv_dict, orient = "index")
    df_csv.to_csv('/proj/wallner/users/x_karst/exjobb/tables/composition_asymmetric_propep.csv')



    plt.scatter(x_list_pep, y_list_pep, label = "Peptide side", color = "#FF7D00", marker = "^")
    plt.scatter(x_list_pro, y_list_pro, label = "Protein side", color = "#FCB775", marker = "o")


    correlation_dict = defaultdict(list)

    for res,value in zip(x_list_pep, y_list_pep):
        correlation_dict[res].append(value)

    for res,value in zip(x_list_pro, y_list_pro):
        correlation_dict[res].append(value)
    print(correlation_dict)

    data1 = []
    data2 = []
    for values in correlation_dict.values():
    ...
    plt.savefig("/proj/wallner/users/x_karst/exjobb/pictures/pro_pep_interface_composition_asym.png")
    plt.show()
...
def plot_number_of_interactions(pro_interface_dict, pep_interface_dict, check):
    ...
    plt.savefig("/proj/wallner/users/x_karst/exjobb/pictures/numb_pro_pep_asym.png", bbox_inches='tight')
    plt.show()
...
def plot_normalized_contacts_and_likelihood(pro_interface_dict, pep_interface_dict, check, fraction_dict_pro, fraction_dict_pep):
    ...
    plt.xlabel("Protein residues")
    plt.ylabel("Peptide residues")
    plt.savefig("/proj/wallner/users/x_karst/exjobb/pictures/norm_numb_pro_pep_asym.png", bbox_inches='tight')
    plt.show()
...
######################################################################################################### Likelihood Gij
    ...
    plt.xlabel("Protein residues")
    plt.ylabel("Peptide residues")
    plt.savefig("/proj/wallner/users/x_karst/exjobb/pictures/gij_propep_asym.png", bbox_inches='tight')
    plt.show()
    ...
    for master_residue, small_dict in pro_interface_dict.items(): #counting the number of contacts
    ...
    df_3_reorder.to_csv('/proj/wallner/users/x_karst/exjobb/protein_peptide_data/gijv_propep_asymmetric.csv', index = False)
    ...
    plt.xlabel("Protein residues")
    plt.ylabel("Peptide residues")
    plt.savefig("/proj/wallner/users/x_karst/exjobb/pictures/gijv_propep_asym.png", bbox_inches='tight')
    plt.show()
    ...
    heatmap = sns.heatmap(df, linewidth = 0.5, vmin = 0 , cmap = "Blues").set_title(title)
...
def main():
    ...
    dataset_file = "/proj/wallner/users/x_karst/exjobb/protein_peptide_data/protein_peptide_dataset.out"
    cut_off = float(cut_off)
    dataset_file= open(dataset_file, "r")


    pairings_file = open("/proj/wallner/users/x_karst/exjobb/trps_contacts_asym.txt", "w")
    ...
    for complex in complexes:
        try:
            check, one_model, within_model, between_models = find_all_contacts(complex, pro_interface_dict, pep_interface_dict, composition_counter_pro, composition_counter_pep, check, one_model, within_model, between_models, cut_off, no_interchain_contacts_file, which_is_protein_dict, pairings_file)
